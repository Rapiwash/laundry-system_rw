...
const Gasto = ({ onClose }) => {
  ...
  const exportToExcel = async () => {
    ...
    const fileName = `Reporte de Egresos : ${nombreMes}, del ${anio}`;

    try {
      const response = await axios.get(
        `${
          import.meta.env.VITE_BACKEND_URL
        }/api/lava-ya/get-reporte-egresos?mes=${mes}&anio=${anio}`
      );
      ...
      if (response.data) {
        ...
        response.data.forEach((item) => {
          const length = Math.max(trasformeDate(item.fecha).length);
        ...
        worksheet.columns = [
          ...
          { header: "Usuario", key: "infoUser", width: 15 },
        ];
